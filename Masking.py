...
def mask_points(img,name):

    # Show image
    plt.imshow (img, cmap='gray')
    plt.title ("Click to define points (up to 200). close the window to finish early.")
    plt.axis ('on')

    # ginput with timeout and maximum of 100 points
    points_raw = plt.ginput (n=200, timeout=0)  # timeout=0 means wait indefinitely
